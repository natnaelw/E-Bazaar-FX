...
class DbClassOrder implements DbClass, DbClassOrderForTest {
    ...
    private void buildUpdateProductQtyQuery() {
    	query =  "  UPDATE productsdb.Product SET totalquantity = totalquantity- "+qty+" WHERE productid = "+ productId+" ;";
        }
    ...
    private void populateOrderHistoryData(ResultSet rs) throws DatabaseException {  	
		...
		try {
			...
			while (rs.next()) {
				...
				order.setCartTotalPrice(Double.valueOf( rs.getString("totalpriceamount")));
    ...
    void submitOrder(ShoppingCart shopCart) throws DatabaseException {
    	...
    	ord.setCartTotalPrice(shopCart.getTotalPrice());
    	this.order = ord;
    	...
    	for(Iterator<CartItem> i = shopCart.getCartItems().iterator(); i.hasNext(); ) {
    	   ...
    	   try {
				Product prod = psf.getProductFromId(item.getProductid());
				prod.setQuantityAvail(prod.getQuantityAvail()-Integer.parseInt(item.getQuantity()));
				psf.updateProduct(prod);
	...
	public List<Order> getOrderHistory() {
	  ...
	  //for testing
	@Override
	public List<Order> readAllOrders(CustomerProfile custProfile)
