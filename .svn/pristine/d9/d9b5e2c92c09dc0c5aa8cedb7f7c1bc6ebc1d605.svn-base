...
public class DbClassShoppingCart implements DbClass {
	...
	public void deleteOldShoppingCart() throws DatabaseException
	...
	// replace the current version of this method in //DbClassShoppingCart with
	// the following
	...
	/*
	 * private void populateTopLevelCart(ResultSet rs) throws DatabaseException
	 * { cartImpl = new ShoppingCartImpl(); Address shippingAddress = null;
	 * Address billingAddress = null; CreditCard creditCard = null; try {
	 * if(rs.next()) { //load shipping address String shipStreet =
	 * rs.getString("shipaddress1"); String shipCity = rs.getString("shipcity");
	 * String shipState = rs.getString("shipstate"); String shipZip =
	 * rs.getString("shipzipcode"); shippingAddress =
	 * CustomerSubsystemFacade.createAddress(shipStreet, shipCity, shipState,
	 * shipZip, true, false);
	 * 
	 * //load billing address String billStreet = rs.getString("shipaddress1");
	 * String billCity = rs.getString("shipcity"); String billState =
	 * rs.getString("shipstate"); String billpZip = rs.getString("shipzipcode");
	 * billingAddress = CustomerSubsystemFacade.createAddress(billStreet,
	 * billCity, billState, billpZip,false, true);
	 * 
	 * //load credit card: createCreditCard(String name, String num, String
	 * type, expDate) String name = rs.getString("nameoncard"); String num =
	 * rs.getString("cardnum"); String type = rs.getString("cardtype"); String
	 * exp = rs.getString("expdate"); creditCard =
	 * CustomerSubsystemFacade.createCreditCard(name, num, type, exp);
	 * 
	 * //load cart cartImpl.setCartId((new
	 * Integer(rs.getInt("shopcartid")).toString()));
	 * cartImpl.setShipAddress(shippingAddress);
	 * cartImpl.setBillAddress(billingAddress);
	 * cartImpl.setPaymentInfo(creditCard);
	 * 
