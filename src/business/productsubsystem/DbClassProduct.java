...
class DbClassProduct implements DbClass,DbClassProductForTest {
	...
	private void buildSaveNewQuery() {
		query = "INSERT into Product "
				...
				+ "VALUES(" + (product.getProductId()>0 ? product.getProductId() : "NULL")
						+ ","
	...
	private void buildUpdateQuery() {
		query = "UPDATE Product "
				+ " SET productname = '" + product.getProductName()
						...
						+ ", priceperunit = "
						+ product.getUnitPrice()						
						+ ", mfgdate='" +product.getMfgDate().format(
								DateTimeFormatter.ofPattern(DATE_PATTERN))+"', description = '" + product.getDescription() + "' WHERE productid = " + product.getProductId();
	...
	private void populateProdList(ResultSet rs) throws DatabaseException {
		...
		try {
			...
			while (rs.next()) {
				...
				product = new ProductImpl(catalog, prodId, productName,
						quantityAvail, unitPrice,
		...
		try {
			if (rs.next()) {
				CatalogImpl catalog = new CatalogImpl(rs.getInt("catalogid"),
						(new CatalogTypesImpl()).getCatalogName(rs
								.getInt("catalogid")));
